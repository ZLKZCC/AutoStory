...
AutoStory 是一个用于写作的AI桌面应用，主要面向长篇小说创作。你可以同时创建多个作品，每个作品里除了包含章节正文外，还包含分卷、大纲、世界线、角色设定等用于长篇创作时可能用到的信息。

这些内容同时也可被 AI 进行编辑和修改；本应用还接入了 Qwen3-TTS，所以章节创作完成之后，还可以将指定的章节转换为有声音频。
